...
$bridge = Read-ModFile 'DevTools\BridgeAdapter\HorticultureBridgeAdapter.cs'
$checks = [ordered]@{
    ...
    'live fixture is transient and cleans only its harvested product' = $bridge -match 'FirstOrDefault\(trait => trait\?\.perennial == true\)' -and $bridge -match 'SetPendingTraits\(new List<VarietyTraitDef>' -and $bridge -match 'item\.def == perennialCrop\.plant\.harvestedThingDef'
}
