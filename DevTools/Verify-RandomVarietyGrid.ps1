...
$checks = [ordered]@{
    ...
    'selection and footprint logic have regression coverage' = $actions -match 'speciesCounts\.Max\(\) - speciesCounts\.Min\(\) <= 1' -and $actions -match 'RandomVarietyGridRegression' -and $bridge -match 'GetMethod\("RandomVarietyGridRegression"'
    'runtime bridge verifies assignments and species balance' = $bridge -match 'HNS_DEV_RANDOM_GRID' -and $bridge -match 'PrepareRandomGridVarieties' -and $bridge -match 'footprintPlants' -and $bridge -match 'assigned=' -and $bridge -match 'minPerSpecies=' -and $bridge -match 'maxPerSpecies=' -and $bridge -match 'mature=' -and $bridge -match 'sown=' -and $bridge -match 'typeof\(List<List<VarietyRecord>>\)'
}
