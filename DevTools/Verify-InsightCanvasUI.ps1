...
$bridgeProject = Get-Content -Raw (Join-Path $root 'DevTools\BridgeTools\HorticultureNovelSeeds.BridgeTools.csproj')
...
$bridge = Get-Content -Raw (Join-Path $root 'DevTools\BridgeTools\HorticultureBridgeTools.cs')
...
$checks = [ordered]@{
    ...
    'bridge companion does not copy framework assemblies' = $bridgeProject -match '<Private>false</Private>' -and $bridgeProject -notmatch '<Private>true</Private>'
    ...
    'RimBridge UI assertions' = $bridge -match 'ux-keyed-guidance' -and $bridge -match 'ux-settings-document'
